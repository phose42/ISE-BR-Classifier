...
pd_all['Title_Body'] = pd_all.apply(
    lambda row: row['Title'] + '. ' + row['Body'] if pd.notna(row['Body']) else row['Title'],
    axis=1
)

# Keep only necessary columns: id, Number, sentiment, text (merged Title_Body)
pd_tplusb = pd_all.rename(columns={
    "Unnamed: 0": "id",
    "class": "sentiment",
    "Title_Body": "text"
})
pd_tplusb.to_csv('Title_Body.csv', index=False, columns=["id", "Number", "sentiment", "text"])
...
# 1) Data file to read
datafile = 'Title_Body.csv'

# 2) Number of repeated experiments
REPEAT = 30 #10

# 3) Output CSV file name
out_csv_name = f'../results_{project}_NB.csv'
...
for repeated_time in range(REPEAT):
    ...
    # Correct method: Use predicted probabilities for accurate ROC/AUC calculation
    y_proba = best_clf.predict_proba(X_test)[:, 1]  # Probability scores for positive class
